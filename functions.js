...
app.post('/createkbz', function(req, res){
  CreateKbz(ObjectId(req.params.kbz),function(err,ret) {
    if(err){
      res.send(500);
    }
    if(ret){
...
var ChangeVariableValue = function(variable_name,value,cb){
  var variable = "variable."+variable_name+".value";
  db.kbz.update({"kbz_id":kbz_id},{$set: {variable: value}}, function(err,ret){
    if(err) {
  ...
  db.variables.find({},function(err,ret){
    ...
    db.kbz.insert(kbz, function(err,newkbz){
      if(err) {
        ...
        CreatePulse(ret[0]._id, function(err,pulse){ 
          if(err) {
            throw err;
          }
          cb(err,newkbz);
