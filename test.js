...
var CreateMember = function(kbz_id,user_id,cb){
  ...
  Member.actions = [];
  Member.status = 1;
  db.members.insert(Member, function(err,member){
    if(err) {
...
var CreateKbz = function(parent_id,user,cb){
  kbz = {};
  kbz.parent_id = parent_id;
  kbz.type = (parent_id ? 1 : 0);
  kbz.actions = [];
  kbz.status = 1;
  kbz.size = (parent_id ? 0 : 1);
  ...
  db.variables.find({},function(err,ret){
    ...
    db.kbz.insert(kbz, function(err,newkbz){
      ...
      CreatePulse(newkbz[0]._id, function(err,pulse){ 
        ...
        CreateMember(newkbz[0]._id,ObjectId(user),function(err,member){ 
          if(err) {
            throw err;
          }
          console.log(member);
          newkbz[0].member = member[0]._id
          cb(err,newkbz[0]);
        });    
      });  
    });
...
var CreateAction = function(parent_id,cb){
  CreateKbz(parent_id,function(err,newkbz){
      if(err) {throw err;
        ...
        db.kbz.update({"_id":parent_id},{$push : {"actions" : {"id": newkbz._id, "type" : 1}}}, function(err,ret){
          if(err) {
...
var CreateProposal = function(kbz_id,initiator,title,body,type,uniq,cb){
  ...
  if (type=="1" || type=="2") {  
    Proposal.member_id = uniq.member;
  }
  if (type=="10" || type=="11") {
    Proposal.statement_id = uniq.statement;
  }
  if (type=="12") {
    Proposal.statement_id = uniq.statement
  ...
  if (type=="20") {
    Proposal.variable_id = uniq.variable;
    Proposal.newvalue = uniq.newvalue;
    Proposal.oldvalue = uniq.oldvalue;
  }
    if (type=="30" || type=="31") {
    ...
    if (type=="31" || type=="32") {
    Proposal.member_id = uniq.member;
    Proposal.action_id = uniq.action;
  }

  db.proposals.insert(Proposal, function(err,ret){
    console.log("create proposal:",err,ret);
    if(err) {
...
var Pulse = function(kbz_id,cb){
  db.kbz.findOne({"_id" : kbz_id },function(err,kbz){
    if(err) {
      throw "Pulse: could not find kbz:"+err;
    }
    console.log("[aa]:" , kbz.pulses.OnTheAir,kbz.pulses.Assigned);
    if (kbz.pulses.OnTheAir) { //in case it is not the first kbz pulse
      db.pulses.findOne({"_id" : kbz.pulses.OnTheAir},function(err,OnTheAir){
        console.log(err,OnTheAir.OnTheAir);
        if(err){
          throw "pulse: could not find OnTheAir:"+err;
        }
        for( pr in OnTheAir.OnTheAir) {
          db.proposals.findOne({"_id" : OnTheAir.OnTheAir[pr].proposal_id},function(err,proposal){
            if(err){throw "ontheair proposal crap: "+err;}
            t = proposal.type;
            type = eval("types."+t.toString());
            console.log("kbz.variables."+type);
            var variable = eval("kbz.variables."+type);
            if(proposal.pro/(proposal.against + proposal.pro) >= variable.value){ /*proposal had passed*/
              ExecuteVertic(proposal,function(err,ret){
                if (err) {
                  throw "Pulse: Proposal id:"+proposal_id+" failed to resolve";
                }
                if(ret){
    ...
    db.pulses.findOne({"_id" : kbz.pulses.Assigned},function(err,Assigned){
      if(err){
        throw "pulse: could not find Pulse Assigned "+err;
      }
      console.log(Assigned);
      for(var pr in Assigned.Assigned) {
        db.proposals.update({"_id" : Assigned.Assigned[pr].proposal_id},{$set : {"status" : "6"}},function(err,ret){
          if(err){
...
//    db.memebers.update({"_id" : ObjectId(proposal.member_id)},{$set : {"status" : 2}},function(err,ret){
    CreateMember(proposal.kbz_id,proposal.initiator,function(err,ret){
      if(ret){
        cb(err,ret);
        return;
      }
  ...
  if(proposal.type == "2"){
    db.memebers.update({"_id" : proposal.member_id},{$set : {"status" : 3}},function(err,ret){
      if(ret){
        cb(err,ret);
  ...
  if(proposal.type == "10"){
    db.statements.update({"_id" : proposal.statement_id},{$set : {"status" : 2}},function(err,ret){
      if(ret){
        cb(err,ret);
        return;
      }
  ...
  if(proposal.type == "11"){
    db.statements.update({"_id" : proposal.statement_id},{$set : {"status" : 3}}, function(err,ret){
      if(ret){
        cb(err,ret);
        return;
      }
  ...
  if(proposal.type == "12"){
    db.statements.update({"_id" : proposal.statement_id},{$push : {"history" : proposal.oldstatement},$set : {"statement" : proposal.newstatement},$inc : {"status":1}}, function(err,ret){
      if(ret){
        cb(err,ret);
        return;
      }
  ...
  if(proposal.type == "20"){
    db.statements.update({"_id" : proposal.variable_id},{$push : {"history" : proposal.oldvalue},$set : {"value" : proposal.newvalue},$inc : {"status":1}}, function(err,ret){
      if(ret){
        cb(err,ret);
        return;
      }
  ...
  if(proposal.type == "30"){
    CreateAction(proposal.kbz_id, function(err,action){
      if(err) { throw err;}
      if(action){
...
app.get('/runtest', function(req, res) {
  CreateKbz(0,req.session.passport.user,function(err,kbz) {
    if(err) {throw err;}
    CreateProposal(kbz._id,ObjectId("52bfbdf04bf1d92c2619a52a"),'i want in','let me in',1,{member : 0},function(err,p1){
      if(err) {throw err;}
      CreateProposal(kbz._id,kbz.member,'our moto','Just kidding',10,{statement : "Dont Be Evil!!"},function(err,p2){
        if(err) {throw err;}
        CreateProposal(kbz._id,kbz.member,'Change Name','WE need to change',20,{variable : "Name" , newvalue : "our House"},function(err,p3){
          if(err) {throw err;}
          Support(kbz._id,p1._id,kbz.member,function(err,ret){
            if(err) {throw err;}
            Support(kbz._id,p3._id,kbz.member,function(err,ret){
              if(err) {throw err;}
              Support(kbz._id,p2._id,kbz.member,function(err,ret){
                if(err) {throw err;}
                PulseSupport(kbz._id,kbz.member,function(err,ret){
                  if(err) {throw err;}
                  CreateProposal(kbz._id,ObjectId("52bfbdf04bf1d92c2619a52a"),'i want in','let me in',1,{member : 0},function(err,p1){
                    if(err) {throw err;}
                    CreateProposal(kbz._id,kbz.member,'our moto','Just kidding',10,{statement : "Dont Be Evil!!"},function(err,p2){
